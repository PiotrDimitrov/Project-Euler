...
namespace pr55 {
    bool isPal(bigNum n){
        for (int i = 0; i < n.digits.size(); i++){
            if (n.digits[i] != n.digits[n.digits.size() - 1 - i]) {return false;}
        }
        return true;
    }
    ...
    bigNum reverse(bigNum n){
        ...
        for (int i = 0; i < n.size(); i++){
            temp.digits[n.size() - 1 - i] = n.digits[i];
        }
        return temp;
    }
}
...
int problem55() {
    int result = 0;
    for (bigNum i = 0; i < 10000; i = i + 1){
        int c = 50;
        bool flag = false;
        bigNum current = i;
        while (c--){
            bigNum temp = current + pr55::reverse(current);
            if (pr55::isPal(temp)) {
                flag = true;
                break;
            }
